...
echo "$($PSQL "SELECT name FROM games LEFT JOIN teams ON games.winner_id = team_id WHERE year=2018 AND round='Final';")"
...
echo "$($PSQL "SELECT year, name FROM games LEFT JOIN teams ON games.winner_id = team_id  WHERE round='Final' ORDER BY year;")"
